...
#define R 0.287  // 기체상수(단위 : J/(mol/K))
#define MAIR 0.0289
...
int main(void)
{
	...
	double sum_Pa = 0;  // 고도에 따른 압력 변화값 (단위 : kPa)
	double nRHO = 0; // 고도에 따른 밀도 변화값 (단위 : kg/m^3)
	...
	while (true)
	{
		if (sys == START) // 시작에 대한 변수일 경우
		{
			...
			if (equ == ENERGY)  // 에너지 방정식에 대한 변수 일 경우
			{
				...
				printf("Constants in Bernoulli's energy equation : %.2lf\n", res);  // 에너지 방정식 결과값 출력
			}
			else if (equ == PRESSURE)  // 압력 방정식에 대한 변수 일 경우
			{
				...
				printf("Speed : %.3lf(m/s)\n", nV);  
				printf("Density : %.3lf(kg/m^3)\n", nRHO);  
				printf("Constants in Bernoulli's pressure equation : %.2lf\n", res);  // 압력 방정식 결과값 출력
			}
...
double get_temperature(double y)  // 고도에 따른 온도 변화 함수 시작
{
	double temp = 0.0;  // 압력 결과값에 대한 변수 지정
	temp = TEM- ((y / 10.0) * (DIM_TEM*10));  // 고도에 따른 온도 변화 공식
	return temp;
}	

double get_rho(double x)  // 고도에 따른 밀도 변화 함수 시작
{
	...
	temp = temp_P / (R * (temp_T + CHANGE_K));  // 고도에 밀도 변화 곻식

	return temp;
}

double get_pressure(double n)  // 고도에 따른 압력 변화 함수 시작
{
	double temp = 0.0;  // 압력 결과값에 대한 변수 지정
	if (n < MAX_ALTITUDE || n >= 0)
	{
		temp = ATM * pow(1.0 - (n * DIM_TEM) / (TEM+CHANGE_K), (G * MAIR) / (8.314 * DIM_TEM));  // 고도에 압력 변화 공식
		return temp;
	}
